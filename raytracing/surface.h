...
//   surface(int num); ///< Constructor for a surface object with num empty triangles
  surface(); 
  surface(Vector<double> Oh); ///< Empty surface object with position Oh
 ...
 // ~surface();
  Vector<double> calcCoM(); /// calculates center of mass
  /**
...
// Die virtuellen Funktionen der ObjectShape-Klasse
   ...
   */
  bool next(const Vector<double> &r, const Vector<double> &k, Vector<double>& p); 
  Vector<double> norm (const Vector<double> &P); ///< Surface normal at point P (There is no check, if P is on the surface. This function is used for internal purposes)
  ...
  Matrix<double> computeInertia(); ///< Calculates the inertia of the object
void setP (Vector<double> r); ///< Set position of the object to r.
  void setP (double x, double y, double z) {setP(Vector<double>(x,y,z)); } ///< Set position of the object to a position vector represented by its components x,y and z
 ...
 // protected:
  Vector<double> currentnorm;  ///< Normal vector of the triangle that was last hit
  ...
  // char *FName ;
void initBounds(Vector<double> &pul, Vector<double> &por); ///< Calculates the bounding box (=circumferent cuboid) but without rotation. The cuboid is represented by the upper right corner (por) and the lower left corner (pul).
 ...
 */
///{
surface operator + (const surface &s, const Vector<double> &v); ///< adds vector v to all corners of the triangle list of the surface object s
surface operator - (const surface &s, const Vector<double> &v); ///< subtracts vector v from all corners of the triangle list of the surface object s
surface operator + (const Vector<double> &v, const surface &s); ///< adds vector v to all corners of the triangle list of the surface object s
surface operator - (const Vector<double> &v, const surface &s); ///< subtracts vector v from all corners of the triangle list of the surface object s
surface operator * (const Matrix<double> &M, const surface &s); ///< multiply every corner of the triangles from surface s by the matrix M
///}
 ...
 */
surface generateHexagonCylinder(double a, double h, Matrix<double> M=UNITY); 
 ...
 * @param M rotation matrix
 */
surface generateHollowHexagonalCylinder (double a, double h, double t, Matrix<double> M=UNITY); 
 ...
 * @param M rotation matrix
 */
surface& generateBullet (double a, double h, double t, Matrix<double> M=UNITY); /// Generiert einen hexagonalen Zylinder mit Seitenlänge a und Höhe h und einer pyramidalen Spitze 
 ...
 * @param N resolution
 */
surface generateEllipsoid (double a, double b, int N, double r0=1.0, Matrix<double> M=UNITY); // generiert einen Rotationsellipsoiden mit Halbachsen a (z-Achse) und b

/**
 * @brief Generates a cylinder with ellipsoidal caps
 * @param  a hei
 */
surface generatePill (double a, double b, double h, int N, double r0=1.0, Matrix<double> M=UNITY);

surface generatePolyBullet(int n, double a, double h, double t, Matrix<double> M = UNITY);
