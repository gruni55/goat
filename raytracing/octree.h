...
template <class T> class Octree
{
 public : 
	  ...
	  */
	 void setBoundingBox(Vector<double> MP, Vector<double> d);
...
template <class T> void Octree<T>::createChilds()
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		P = BBox.P + Vector<double>(sx*BBox.d[0] / 4.0, sy*BBox.d[1] / 4.0, sz*BBox.d[2] / 4.0);
		d = BBox.d / 2.0;
...
template <class T> void Octree<T>::setBoundingBox(Vector<double> MP, Vector<double> d)
{
	BBox = Box(MP, d,1.0);
}
