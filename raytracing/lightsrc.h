  ...
  void setD(double D) ///< sets the width of the light source
   {
	...
	this->D=D;
	reset();} ///< sets the width of the light source (this resets also the ray counter)
  void setk(const Vector<double> &k); ///< sets the main direction of the light source
  Vector<double> getk() { return k; } ///< returns the main direction of the light source
  int getAnzRays() { return N; } ///< returns the number of rays (per direction in space)
  void setAnzRays(int N) ///< sets the number of rays (per direction in space)
{
   this->N=N;
   density = D / ((double)N);
   reset();
}
  void setPol(Vector<std::complex<double> > pol) { Pol = pol; } ///< sets the polarisation 
  void setPos(Vector<double> P); ///< sets the position of the light source. This is the center of the square area of the light source
  Vector<double> getPos() {return Pos;}  ///< returns the position of the light source. This is the center of the square area of the light source
...
// protected :
  	Vector<double> Pos; ///< position of the light source (center of the square area of the light source)
	int type;           ///< type of the light source
	 double P0;        ///< power
    double density;     ///< ray density, i.e. distance between two neighboring rays
    Vector<double> k;   ///< main direction of the light source   
	...
	Vector<std::complex<double> > Pol; ///< polarisation (default: (0.0, 1.0, 0.0)
	Vector<std::complex<double> > Pol2; ///< second polarisation (used by IRay)
	...
	Vector<double> e1,e2;  ///< unit vectors that span the light source area 
...
public:
	 ...
	 */
	LightSrcPlane (Vector<double> Pos, int N, double wvl,  double D=100.0,  Vector<std::complex<double> > Pol=Vector<std::complex<double> >(0.0,1.0,0.0), int raytype=LIGHTSRC_RAYTYPE_IRAY, double r0 = 100.0 ); ///<constructor with the following parameters : Pos: position of the light source, k : main direction of the light source, N : number of rays per direction(total number of all rays is N* N)
	 LightSrcPlane(const LightSrcPlane &); ///< copy Constructor
	~LightSrcPlane(void){}; ///< Destructor, not needed
	int next(RayBase* ray);  ///< gives the next ray for the following calculations 
...
public:
	...
	int next(IRay &S);	
	int next(tubedRay &S);	
	 // void initElectricFieldGauss(Vector<double> &P, Vector<double> &k, Vector<std::complex<double> > &E); ///< Initialisierung eines Teilstrahls
	LightSrcGauss(void);
	 ...
	 */
    LightSrcGauss (Vector<double> Pos,  int N, double wvl, double w0, Vector<double> focuspos, double D=1.0,  Vector<std::complex<double> > Pol=Vector<std::complex<double> >(0.0,1.0,0.0), int raytype=LIGHTSRC_RAYTYPE_IRAY, double r0=1.0);
	void setW0(double w0) {this->w0=w0; calcz0(); reset();} ///< sets the beam's waist 
        Vector<double> getFocuspos() {return focuspos;} ///< returns the position of the focus
	void setFocuspos(Vector<double> fp) {focuspos=fp; f=abs(Pos-focuspos); reset(); } ///< sets the focus position to fp
        void setPos(Vector<double> pos) {Pos=pos; f=abs(Pos-focuspos); reset(); } ///< sets the position of the light source to pos
	...
	void setk(Vector<double> k) {this->k=k; reset(); }	///< sets the main direction of light source 
...
protected :
	...
	Vector<double> focuspos; ///< focal position	
	double z0;				 ///< Rayleigh length (for internal use only)
	Vector<double> k;		 ///< direction of the gaussian beam
