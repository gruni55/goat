...
public: 
  ...
  Vector<std::complex<double> > getE() { return E1; }
  /**
   ...
   */
  IRay reflect(Vector<double> n, std::complex<double>  n1, std::complex<double>  n2); 
  /**
   ...
   * \param n2 Refractive index (transmitted side)
   */
  void refract(Vector<double> N, std::complex<double>  n1, std::complex<double>  n2);
  ...
  void setk(const Vector<double> &K) { k=K; } ///< Sets the direction of the ray 
  void setP(const Vector<double> &p) { P=p; } ///< Current position of the ray
  void setiR (int i) { iR=i; } ///< Sets the current reflexion counter (for internal use only)
 // void setGetunnelt (bool v) { getunnelt=v; } 
  Vector<double> getk() { return k; } ///< Returns the direction of the ray
  Vector<double> getP() { return P; } ///< Returns the current position of the ray
   ...
   */
  bool checkObjectIntersection(int &Index, Vector<double> &Pmin); 
...
public:
	Vector<std::complex<double> > Pol1() {return E1/abs(E1); } ///< direction of the first polarisation
	Vector<std::complex<double> > Pol2() {return E2/abs(E2); } ///< direction of the second polarisation
 
  Matrix<std::complex<double> > Fresnel_reflect (double alpha,std::complex<double>  n1, std::complex<double>  n2); ///< returns Fresnel matrix for the reflection calculation (alpha: angle of incidence, n1,n2 are the refractive indices)
  Matrix<std::complex<double> > Fresnel_trans (double alpha,std::complex<double>  beta, std::complex<double>  n1, std::complex<double>  n2); ///< returns Fresnel matrix for the transmission calculation (alpha: angle of incidence, n1,n2 are the refractive indices)
  void initElectricField (const Plane& Eb, const Vector<std::complex<double> >& Pol,const int numOfRays=1); ///< initialises the electric field with help of the Plane Eb and the polarisation Pol, numOfRays can be omitted. (Here, only one polarisation is given, the other is skipped)
  void initElectricField (const Vector<std::complex<double> >& PolS, const Vector<std::complex<double> > &PolP, const int AnzRays); ///< initialises the electric field vectors with help of polarisation vectors PolS and PolP, numOfRays can be omitted
  void initElectricField (const Plane& Eb, const Vector<std::complex<double> >& Pol1, const Vector<std::complex<double> >& Pol2, const int AnzRays);
  /**
   ...
   */
  void initElectricFieldGauss (double sigma2, Vector<double> focuspos,  Vector<std::complex<double> > Pol);
void initElectricFieldGauss (const Plane& Eb,
                                   const Vector<std::complex<double> >& PolS,
                                   const Vector<std::complex<double> > &PolP,
  ...
  void initElectricFieldGauss (Vector<std::complex<double> > &Pol, Gauss g); ///< Initialises the ray for gaussian beam with help of the plane Eb and the polarisation vector Pol (second polarisation is not used, E2 is set to E1). The parameters for the gaussian beam are stored in g
  double cross (const Vector<double> P10, const Vector<double> P11, const Vector<double> P20, const Vector<double> P21); 

  Vector<double> crossPlane (const Vector<double> Pe, const Vector<double> n); ///< calculates the intersection point between the ray and a plane, which is defined by the vector P and the normal n
  /**
   ...
   */
  Vector<double> intersectRect (const Vector<double> P, const Vector<double> e1, const Vector<double> e2); 
  

  
  Vector<double> P; ///< current position 
  Vector<double> k; ///< current direction
  Vector<std::complex<double> > E1,E2; ///< electric fields 
