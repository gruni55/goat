...
public:
    ...
    double distance(const Vector<double> &p, const Vector<double> &k); ///< calculates the distance between a ray, described by the position vector p and its directional vector k 
	bool next(const Vector<double> &p, const Vector<double> &k, Vector<double> &pout); ///< calculates the next crossing point between a ray represented by the position 
	Vector<double> norm(const Vector<double> &p); ///< normal vector at the position p
	bool isInside(const Vector<double> &Ps); ///< returns true, if Ps is inside the box
	...
	void calcDiag() ///< calculates the longest diagonal inside the box
	{
		...
		diag[1] = Vector<double>(-diag[0][0], diag[0][1], diag[0][2]);
		diag[2] = Vector<double>(-diag[0][0], -diag[0][1], diag[0][2]);
	}

	void setP(Vector<double> r) ///< sets the position P and the corresponding bounds
	 { 
		  P = r; 
		  bounds[0] = P - d / 2.0;
		  bounds[1] = P + d / 2.0;
	 }
	void setP(double x, double y, double z) { setP(Vector<double>(x, y, z)); } ///< sets the position P and the corresponding bounds
	void setD(Vector<double> D) ///< sets the extensions in x-, y- and z-direction
	{ 
		d = D; 
		bounds[0] = P - d / 2.0;
	    bounds[1] = P + d / 2.0;
	...
	Vector<double> bounds[2];  ///< positions of the two opposite corners  
	Vector<double> d; ///< extensions of the box in x-, y- and z-direction
	Vector<double> diag[3]; ///< diagonal 
        Vector<double> calcCoM () { return dzero;}  
