...
bool Ellipsoid::next(const Vector<double> &Ps, const Vector<double> &K,
                     Vector<double> &pout)
{
 bool innen;
 double A,B,C;
...
Vector<double> Ellipsoid::norm (const Vector<double> &p)
{
 Vector<double> ps,n;
 ps=H*(p-P);
...
/* Ellipsoid & Ellipsoid::operator = (const Ellipsoid& e)
 ...
 bool Ellipsoid::isInside (const Vector<double> &p)
 {
  Vector<double> h;
  h=ediv (p,r);
  return abs2(h)<1.0;
...
void Ellipsoid::binWrite (ofstream &os)
{
 P.binWrite(os);
 H.binWrite(os);
 R.binWrite(os);
...
void Ellipsoid::binRead (ifstream &is)
{
 type=OBJECTSHAPE_ELLIPSOID;
 P.binRead(is);
 H.binRead(is);
...
ostream &operator << (ostream &os,Ellipsoid E)
{
 os << "%r=" << E.r << endl;
 os << "%P=" << E.P << endl;
...
void Ellipsoid::setr (Vector<double> &r) 
  { 
    this->r=r; 
    r_2=Vector<double> (1.0/(r[0]*r[0]),1.0/(r[1]*r[1]),1.0/(r[2]*r[2])); 
  }
 ...
 Matrix<double> Ellipsoid::computeInertia()   
 {
	 Matrix<double> I;
