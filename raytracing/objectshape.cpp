...
ObjectShape::ObjectShape (const Vector<double> &P,
        complex<double>  n,
...
void ObjectShape::setMatrix (Matrix<double> H)
{
	this->H=H;
	R=invert(H);
	// R=invert(H);
...
void ObjectShape::rotate (Vector<double> A, double phi)
{
 R=rotMatrix (A,phi);
 H=invert(R);
 e[0]=H*ex;
...
{  
 Matrix <double> Dx,Dy,Dz;
 Dx=rotMatrix (ex,alpha);
 Dy=rotMatrix (ey,beta);
 Dz=rotMatrix (ez,gamma);
...
Matrix<double> computeInertia(ObjectShape *F)
{
    switch (F->type)
	{
...
void ObjectShape::setCenter(Vector<double> P)
{
        switch (type) 
        {
