...
Box::Box(const Vector<double> &P,
	...
	const Vector<double> Ez) : ObjectShape(P,n,alpha,Ex,Ey,Ez)
    {
		bounds[0] = P - d / 2.0;
		bounds[1] = P + d / 2.0;
		this->d = d;
...
double Box::distance (const Vector<double> &ps, const Vector<double> &K)
{
...
*/
Vector<double> Box::norm(const Vector<double> &ps)
{
	Vector<double>p = ps - P;
	p = H*p;
/*	if (fabs(p[0] - d[0] / 2.0) < EPS) { cout << "n=" << ex << endl; return ex; }
	...
	if (fabs(p[0] - d[0] / 2.0) < EPS) return R*ex;
	if (fabs(p[0] + d[0] / 2.0) < EPS) return -R*ex;
	if (fabs(p[1] - d[1] / 2.0) < EPS) return  R*ey;
	if (fabs(p[1] + d[1] / 2.0) < EPS) return -R*ey;
	if (fabs(p[2] - d[2] / 2.0) < EPS) return R*ez;
	if (fabs(p[2] + d[2] / 2.0) < EPS) return -R*ez;  
//        cout << "n=" << zero << "   ps=" << ps << "   p=" << p << "   d=" << d << "    p[0] - d[0] / 2.0=" << p[0] - d[0] / 2.0 << endl;
	return one;
}

bool Box::isInside(const Vector<double> &Ps)
{
	Vector<double> p = P - Ps;
	p = H*p;
...
void Box::initQuad()
{
	Vector<double> b0 = H*bounds[0];
	Vector<double> b1 = H*bounds[1];
	por = Vector<double>(max(b0[0],b1[0]), max(b0[1], b1[1]),max(b0[2], b1[2]))+P;
	pul = Vector<double>(min(b0[0], b1[0]), min(b0[1], b1[1]), min(b0[2], b1[2]))+P;
}
