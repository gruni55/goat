...
void LightSrc::setk(const Vector<double> &k)
{
	this->k = k / abs(k);
	e1 = k % ez;
	if (abs(e1) < 1E-10) e1 = k % ex;
	e2 = k % e1;
	e1 = e1 / abs(e1);
	e2 = e2 / abs(e2);
}
...
LightSrcPlane::LightSrcPlane (Vector<double> Pos, int N, double wvl, double D, Vector<complex<double> > Pol, int raytype, double r0) : LightSrc()
{
	e1=ex;
	e2=ey;
...
LightSrcGauss:: LightSrcGauss (Vector<double> Pos,  int N, double wvl, double w0, Vector<double> focuspos, double D,  Vector<complex<double> > Pol, int raytype, double r0) : LightSrc()
/**
...
/*
void LightSrcGauss::initElectricFieldGauss(Vector<double> &P, Vector<double> &n, Vector<complex<double> > &E)
{
...
{
	os << "% Anzahl Objekte:" << ls->numObjs << endl;
	for (int i=0; i<ls->numObjs; i++)
	{		
		os << "%------------------- Objekt " << i << " -----------------" << endl;
		os << "% P=" << ls->Obj[i]->P << endl;
		if (ls->Obj[i]->type==OBJECTSHAPE_ELLIPSOID)
		{
			Ellipsoid *e=(Ellipsoid *)ls->Obj[i];
			os <<  "% Halbachsen= " << e->r << endl;			
		}
		os << "% Brechungsindex n=" << real(ls->Obj[i]->n) << "+ i*" << imag(ls->Obj[i]->n) << endl;
                os << "% -----------------  ENDE ----------------" << endl; 
                 
	}
        os << "% Pos=" << ls->getPos() << endl; 
	os << "% D=" << ls->D << "   k=" << ls->getk() << "  N=" << ls->N << endl;
	os << "% density=" << ls->density << "  n0=" << ls->n0 << "   Pow=" << ls->P0 << endl;
        switch (ls->type)
        {
         case LIGHTSRC_SRCTYPE_GAUSS : os << "Fokuspos=" << ((LightSrcGauss *)ls)->getFocuspos() << endl; break; 
        };
	return os;
}
