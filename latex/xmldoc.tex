...
A calculation can be carried out directly via the GOAT library. Sometimes it is useful to save the scene information externally. A separate GOAT::XML::xmlReader class is available for this purpose. It is based on an XML file, the structure of which is described in more detail in this documentation. 
 ...
 \hline
 nReflex & number of reflections & integer & 1 
    \end{tabular} 
...
\subsubsection{Point source (MC)}
type: \textbf{point\_mc} \newline
This light source emitts rays randomly to every direction. The range can be restricted to a cone, given by a certain direction vector and an opening angle thetaMax. 
 \begin{longtable}{p{2cm}|m{3.5cm}|m{3.0cm}|p{1.7cm}}
Parameter name & description  & possible value & default value\\
 \hline
 thetaMax & opening angle & double (0..$\pi$) & $\pi$  
 \end{longtable}
...
\subsubsection{Objects with roughness} 
 One can also create objects with roughness. In this process, the surface normal is varied randomly. You can choose between different distribution types. In addition, you can limit the angle range within which the normal is varied. To do this, set the value of thetaMax. To define an object as "rough" an additional parameter "isrough" has to be set to "true". Than, the corresponding roughness parameters can be set in an section "Roughness".
 
  \vspace{1em} 
\begin{tabular}{p{2cm}|m{3.0cm}|m{3.5cm}|p{1.7cm}}
 Parameter  & description  & possible value & default value\\
 \hline
 type & Type of the random distribution & "gaussian",\newline"cosine\_cone",\newline"uniform\_cone" & gaussian \\
 \hline
 thetaMax & maximal variation angle & 0 $\le $ thetaMax $\le \nicefrac{\pi}{2}$ & $\nicefrac{\pi}{2}$ \\
 \hline
 sigma\footnote{for gaussian distribution only} & standard deviation (in radians) & $\ge$ 0 & 0  
 \end{tabular}
 ...
 \vspace{1em} 
\begin{tabular}{p{2cm}|m{3.5cm}|p{3.0cm}|p{1.7cm}}
 Parameter  & description  & possible value & default value\\
 \hline
 ID & unique identifier & string & -- \\
 \hline
...
\end{tabular}
 ...
 type: \textbf{kirchhoff} \newline
 Especially in focal regions, pure raytracing may fail. Therefore, a Kirchhoff-Integral solution is introduced. 
 \begin{equation}
  \vec{E}(\vec{r})=\sum_l \int_{\text{Detector}_l} \vec{E}^i_0(\vec{r}_l) e^{i\vec{k}(\vec{r}-\vec{r}_l)} d\vec{r}_l 
 \end{equation}
 Each Kirchhoff detector uses at least one plane Detector as source. So in the XML a Kirchhoff detector has additional link-entries in the following form:
   \lstset{language=XML}
 \begin{lstlisting}
 <Link ID="[Detector-ID]"/>
   \end{lstlisting}
\begin{tabular}{p{2cm}|m{3.5cm}|m{3.0cm}|p{1.7cm}}
 Parameter  & description  & possible value & default value\\
 \hline 
 wavelength & wavelength for the calculation & double & 1.0 \\
 \hline
 d & width of the quadratic detector & double & \\
 \hline  
 \end{tabular}
 \subsection{Angular spectrum detector}
 type: \textbf{angular\_spectrum} \newline
 When propagating the electric field distribution from a plane detector Kirchhoff can be used, this is accurate but often very time consuming. There a detector is introduced, which propagates the field from the source detector to the target detector. Since this get use of FFT, the underlying calculation process is very efficient. For the propagation, the wavelength has to given as a parameter.
Like for Kirchhoff detectors, each angular spectrum detector has to be linked to a plane detector via 
 \begin{lstlisting}
 <Link ID="[Detector-ID]"/>
   \end{lstlisting}
  the size of the angular spectrum detector must be at least as large as the source detector, also the resolution must be the same, therefore only the size is given and the number of pixels is calculated from the resolution of the source. If the angular spectrum detector is larger than the source. If the detector is quadratic, one can give "d" for the edge length otherwise "d1" for the width and "d2" for the height. The parameter "d" has priority. 
...
\begin{tabular}{p{2cm}|m{3.5cm}|m{3.0cm}|p{1.7cm}}
 ...
 wavelength & wavelength for the calculation & double & 1.0 \\
 \hline
 d & width and height, if its quadratic & double &\\ 
 \hline
 d1 & width of the detector & double & 1 \\
\hline 
 d2 & height of the detector & double & 1 \\
 \hline  
 \end{tabular}   
...
% \begin{tabular}{c|m{3cm}|m{3cm}|c}
 ...
 \hline
  ...
  \hline
  numWavelengthsPerRange & Number of wavelengths considered in each spectral range & integer $>$ 0 & 200  \\
  \hline
...
% \end{tabular}
 ...
 \hline 
 Air & air \\
 Glass & glass (always refractive index 1.5) \\
 N-BK7 glass & bk7 \\
