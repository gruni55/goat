...
int main(int argc, char** argv)
{
	...
	double r = 50.0; 				        // radius of the sphere
	GOAT::maths::Vector<double> Pos(0, 0, 0);   		// Position of the sphere
	GOAT::maths::Vector<double> dim(r/2.0, r, r);		    // set its dimensions
	...
	GOAT::maths::Vector<double> Box2Dim(300, 100, 100);
	...
	GOAT::maths::Vector<double> LSPos(-510, 0, 0);       // Position of the light source
 	int numRays = 1000000;                                      // number of rays (per direction => in total numRaysxnumRays ) 
        double LSdim = 100;                             // Edge length of the light source
...
//	nList.push_back(GOAT::raytracing::n_Vacuum);                      // function for the surroundings 
	...
	double t = 400.0;                                                 // set the time (in fs) at which you want to calculate the fields
	pc.field(t); 	                                                   // make calculation
	GOAT::raytracing::saveabsE(pc.trafo.SAres, "fieldabs1.dat", 0);
